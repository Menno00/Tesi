...
def publish_robot_info():
    ...
    info_robot_message = InfoRobotMessage(robot_descriptor.robot_id, robot_descriptor.robot_speed,
                                          robot_descriptor.robot_action, robot_descriptor.robot_current_action,
                                          robot_descriptor.robot_operator_id).to_json()
...
def publish_robot_telemetry():
    ...
    old_joints = robot_descriptor.robot_joints
    while not flag_pick_and_place:

        robot_descriptor.robot_joints = robot.arm.get_joints()
        if old_joints != robot_descriptor.robot_joints:
            telemetry_message = TelemetryMessage(robot_descriptor.robot_id, robot_descriptor.robot_speed, robot_descriptor.robot_joints,
                                                 robot_descriptor.robot_end_effector).to_json()
...
def pick_n_place(pick_pose, place_pose):
    ...
    robot_descriptor.robot_end_effector = False
    # Going to picking place and closing gripper
    robot.arm.move_pose(pick_pose)
    robot.tool.grasp_with_tool()
    robot_descriptor.robot_end_effector = True
    # Raising
    robot.arm.move_pose(pick_pose_high)
...
def pick_pose_task():
    ...
    for pick_pose in action_robot_message.pick_pose_list:
        if not stop_movement:
            ...
            pick_pose_number += 1
            time.sleep(robot_descriptor.robot_speed)
    ...
    robot_descriptor.robot_operator_id = None
    robot_descriptor.robot_current_action = None
...
def on_message(client, userdata, message):
    ...
    if robot_descriptor.robot_current_action is None:
        message_payload = str(message.payload.decode("utf-8"))
        action_robot_message = ActionRobotMessage(**json.loads(message_payload))

        if action_robot_message.action_type == 'task' \
                and action_robot_message.task_name in robot_descriptor.robot_action:
            stop_movement = False
            print(f"Received IoT Message: Topic: {message.topic} Payload: {message_payload}")
            publish_robot_event("New Mission", message_payload)

            robot_descriptor.robot_current_action = action_robot_message.task_name
            robot_descriptor.robot_operator_id = action_robot_message.operator_id
            robot_descriptor.robot_speed = action_robot_message.speed
            #robot.arm.set_arm_max_velocity(robot_descriptor.robot_speed)
            publish_robot_info()

            pick_pose_thread = Thread(target=pick_pose_task)
            pick_pose_thread.start()
            return

        elif action_robot_message.action_type == 'stop' or action_robot_message.action_type == 'speed':
        ...
        if second_action_robot_message.action_type == 'stop':
            ...
            robot_descriptor.robot_speed = second_action_robot_message.speed
            #robot.arm.set_arm_max_velocity(robot_descriptor.robot_speed)
            publish_robot_info()
...
robot_descriptor = RobotDescriptor("NiryoNed", "10.10.10.10", ["Faro", "Albero"])
time.sleep(10)
